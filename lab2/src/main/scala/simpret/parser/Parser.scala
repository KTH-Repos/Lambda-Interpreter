...
object Parser extends Parsers {
  ...
  */
  ...
  def term1: Parser[AST] = {
    ...
    term2 ~ rep(plus_term2) ^^ { case t1 ~ t2 => (t2.foldLeft(t1)) { (acc,f) => f(acc) } }
  }
  ...
  def term2: Parser[AST] = {
    ...
    (fix_app | term3) ~ rep(app_term3) ^^ { case t1 ~ t2 => (t2.foldLeft(t1)) { (acc,f) => f(acc) } }
  }
  ...
  def term3: Parser[AST] = {
    ...
    term4 ~ rep(cons_term4) ^^ {
      case t1 ~ t2 =>
        ...
        (eFuns.foldLeft(er)) { (acc, f) => f(acc) }
    }
  }

  def term4: Parser[AST] = {
    rep(TOKMINUS) ~ term5 ^^ { case ml ~ t => (ml.foldLeft(t)) { (acc, _) => UMinExp(acc) } }
  }
  ...
  def term5: Parser[AST] = {
    ...
    atom ~ rep(tuple_record_proj) ^^ { case t1 ~ t2 => (t2.foldLeft(t1)) { (acc,f) => f(acc) } }
  }
  ...
  def list: Parser[AST] = {
    TOKSQBL ~ opt(stmt ~ rep(comma_stmt) ~ TOKVBAR) ~ type_exp ~ TOKSQBR ^^ {
      ...
      case _ ~ Some(e ~ es ~ _) ~ ty ~ _ => ((e::es).foldRight(NilExp(ty):AST)) { ConsExp.apply }
    }
  }
  ...
  def record: Parser[AST] = {
    ...
    TOKCURL ~ record_item ~ rep(comma_record_item) ~ TOKCURR ^^ {
      case _ ~ it1 ~ its ~ _ =>
        RecordExp(((it1::its).foldLeft(Map.empty:Map[String,AST])) {
          case (m, (name,e)) =>
  ...
  def type_exp: Parser[ASTTY] = positioned {
    ...
    type_atom ~ rep(arrow_type_atom) ^^ { case t1 ~ t2 =>
      ...
      (tyFuns.foldLeft(tyr)) { (acc,f) => f(acc) }
    }
  }
  ...
  def type_atom: Parser[ASTTY] = {
    ...
    def record_type: Parser[ASTTY] = {
      ...
      TOKCURL ~ record_item_type ~ rep(comma_record_item_type) ~ TOKCURR ^^ {
        case _ ~ it1 ~ its ~ _ =>
          RecordTy(((it1::its).foldLeft(Map.empty:Map[String,ASTTY])) {
            case (m, (name,ty)) =>
              if (m.keySet.contains(name)) {
